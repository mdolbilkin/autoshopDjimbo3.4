...
@dp.callback_query_handler(text="promocode", state="*")
async def promocode(call: CallbackQuery, state: FSMContext):
    await PromoCodeForm.waiting_for_promo_code.set()
    await call.answer("Введите промокод:")


@dp.message_handler(state=PromoCodeForm.waiting_for_promo_code)
async def process_promo_code_handler(message: Message, state: FSMContext):
    # Get user's answer
    promo_code = message.text
    con = sqlite3.connect("C:\\Users\\92552\\PycharmProjects\\autoshopDjimbo3.4\\tgbot\data\\database.db")
    cur = con.cursor()
    a = cur.execute("SELECT * FROM promocodes").fetchall()
    check = False
    for i in a:
        if i[0] == promo_code and str(message.from_user.id) not in i[2] and i[3] > 0:
            await message.answer(f"Промокод успешно активирован! Вам зачисленно {i[1]} рублей.")
            b = cur.execute("SELECT * FROM storage_users").fetchall()
            for j in b:
                if int(j[1]) == message.from_user.id:
                    cur.execute("UPDATE storage_users SET user_balance = ? WHERE user_id = ?", (j[4] + i[1], j[1]))
            cur.execute(f"UPDATE promocodes SET users = ? WHERE promo = ?",
                        (i[2] + ', ' + str(message.from_user.id), i[0]))
            cur.execute(f"UPDATE promocodes SET hmch = ? WHERE promo = ?",
                        (i[3] - 1, i[0]))
            con.commit()
            check = True
        elif i[0] == promo_code and str(message.from_user.id) in i[2]:
            ...
            check = True
    if not check:
        await message.answer("Вы ввели неправильный промокод!")
